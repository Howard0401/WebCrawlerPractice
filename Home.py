...
def gogo():
  ...
  videoTable = pandas.DataFrame({"Name": video.name, "Img" : video.img, "Min": video.minPrice, "Max": video.maxPrice})
  lifeTable = pandas.DataFrame({"Name":  life.name, "Img" :  life.img, "Min":  life.minPrice, "Max":  life.maxPrice})
  kitchenTable = pandas.DataFrame({"Name": kitchen.name, "Img" : kitchen.img, "Min": kitchen.minPrice, "Max": kitchen.maxPrice})
  healthTable = pandas.DataFrame({"Name": health.name, "Img" : health.img, "Min": health.minPrice, "Max": health.maxPrice})
  massageTable = pandas.DataFrame({"Name": massage.name, "Img" : massage.img, "Min": massage.minPrice, "Max": massage.maxPrice})

  videoOutput = json.loads(videoTable.to_json(orient='records', force_ascii=False))
  lifeOutput = json.loads(lifeTable.to_json(orient='records', force_ascii=False))
  kitchenOutput = json.loads(kitchenTable.to_json(orient='records', force_ascii=False))
  healthOutput = json.loads(healthTable.to_json(orient='records', force_ascii=False))
  massageOutput = json.loads(massageTable.to_json(orient='records', force_ascii=False))

  output = {'video':videoOutput, 'life': lifeOutput , 'kitchen': kitchenOutput, 'health': healthOutput, 'message': massageOutput}
